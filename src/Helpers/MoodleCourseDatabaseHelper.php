...
class MoodleCourseDatabaseHelper
{
    ...
    public static function getAllMoodleCourses()
    {
        ...
        $databaseHelper->query("SELECT id, fullname, shortname, category  FROM mdl_course ORDER BY id");
    ...
    {
        $databaseHelper = new DatabaseHelper("moodle");
        $databaseHelper->query("SELECT id, fullname, shortname, category FROM mdl_course WHERE id = :id LIMIT 0,1");
        $databaseHelper->bind(':id', $id);
        $moodleCourse = $databaseHelper->single();
        if ($databaseHelper->rowCount() === 0) {
            return 0;
        }
        return new MoodleCourse($moodleCourse['id'], $moodleCourse['fullname'], $moodleCourse['shortname'], $moodleCourse['category']);
    }
}